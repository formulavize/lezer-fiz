...
import { insertSemi, spaces, newline,
  LineComment, BlockComment, Unit, cssIdentifier } from "./parser.terms.js"

const space = [9, 10, 11, 12, 13, 32, 133, 160, 
  5760, 8192, 8193, 8194, 8195, 8196, 8197, 8198, 8199, 8200,
  8201, 8202, 8232, 8233, 8239, 8287, 12288]

const percent = 37, dash = 45, underscore = 95, braceR = 125
...
export const cssIdentifierToken = new ExternalTokenizer((input) => {
  for (let inside = false, dashes = 0, i = 0;; i++) {
    ...
    if (isAlpha(next) || next == dash || next == underscore || (inside && isDigit(next))) {
      ...
      if (inside)
        input.acceptToken(cssIdentifier)
      break
    }
  }
})